...
import {ArrowUp,Leaf,MessageCircle,PanelRightClose,Sparkles} from 'lucide-react';
...
function AssistantPanel(){
  ...
  return <>
    ...
    <aside className={`home-assistant-drawer ${open?'open':''}`} aria-hidden={!open} aria-label="SiteSafe home assistant">
      ...
      <form className="home-assistant-input" onSubmit={submit}><textarea value={input} onChange={event=>setInput(event.target.value)} placeholder={agentId?'Ask a question...':'Add NEXT_PUBLIC_ELEVENLABS_AGENT_ID to enable chat'} rows={2} disabled={!agentId||loading} onKeyDown={event=>{if(event.key==='Enter'&&!event.shiftKey){event.preventDefault();void send(input)}}}/><button type="submit" disabled={!input.trim()||loading||!agentId} aria-label="Send message"><ArrowUp size={17}/></button></form>
